...
def _hijacked_get_learned_conditioning(model, prompts, total_steps, *args, original_function, **kwargs):
    ...
    tensor_builders = _parse_tensor_builders(prompts, real_total_steps, is_hires)
...
def _parse_tensor_builders(prompts, total_steps, is_hires):
    tensor_builders = []

    for prompt in prompts:
        expr = prompt_fusion_parser.parse_prompt(prompt)
        tensor_builder = interpolation_tensor.InterpolationTensorBuilder()
        expr.extend_tensor(tensor_builder, (0, total_steps), total_steps, dict(), is_hires)
